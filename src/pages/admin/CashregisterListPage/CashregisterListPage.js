...
import './CashregisterListPage.css';
import cashregisterApi from '../../../services/apiServices/cashregisterApi';
...
const CashregisterListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await cashregisterApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await cashregisterApi.export({ term: term, startDate, endDate });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Id}>
                        ...
                        <td data-label='Balance'><span>{item.Balance}</span></td>
                    </tr>
                ))}
                </tbody>
            </table>
...
export default CashregisterListPage;
