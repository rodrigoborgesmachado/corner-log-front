...
import './MailmessageListPage.css';
...
const MailmessageListPage = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            try {
                const response = await mailmessageApi.getPaginated({ page, quantity, orderBy, term: searchTerm, startDate, endDate, include: "" });
    ...
    const exportFunction = async ({term}) => {
        try {
            dispatch(setLoading(true));
            const response = await mailmessageApi.export({ term: term, startDate, endDate });
            ...
            <table className="admin-table">
                ...
                {items.map((item) => (
                    <tr key={item.Code}>
                        ...
                        <td data-label='PersonId'><span>{item.PersonId}</span></td>
                        <td data-label='Subject'><span>{item.Subject}</span></td>
                        <td data-label='Body'><span>{item.Body}</span></td>
                        <td data-label='To'><span>{item.To}</span></td>
                        <td data-label='CC'><span>{item.CC}</span></td>
                        <td data-label='Retries'><span>{item.Retries}</span></td>
                        <td data-label='MailMessageFamilyStatus'><span>{item.MailMessageFamilyStatus}</span></td>
